...
const BlogCard: React.FC<BlogPost> = ({
  ...
  return (
    <motion.div
      ...
      className="group bg-[#181818] rounded-xl overflow-hidden border border-amber-900/50 hover:border-amber-500/30 transition-all duration-300"
    >
      <a href={blogPath} className="block">
        <div className="relative h-48 md:h-56 lg:h-64 overflow-hidden">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="absolute bottom-3 right-3 p-2 bg-amber-900/30 rounded-full hover:bg-amber-800/40 transition-colors shadow-sm"
            onClick={(e) => e.preventDefault()}
          >
            <FiBookmark className="text-amber-100 hover:text-amber-400" />
          </motion.button>
        </div>
        ...
        <div className="p-5">
          <div className="flex justify-between items-start mb-3">
            <motion.span
              ...
              className="inline-block px-3 py-1 text-xs font-semibold bg-gradient-to-r from-amber-500 to-red-600 text-white rounded-full"
            >
              {category}
            </motion.span>
            <span className="text-xs text-[#d1c9c3]">{date.split(",")[0]}</span>
          </div>
          ...
          <motion.p
            className="text-[#d1c9c3] mb-4 line-clamp-2 text-sm"
          ...
          <div className="flex justify-between items-center">
            <div className="flex flex-wrap items-center gap-4 text-xs text-[#d1c9c3]">
              <motion.div className="flex items-center" whileHover={{ x: 3 }}>
                <FiClock className="mr-1.5 text-amber-400" />
                {readTime}
              </motion.div>
              <motion.div className="flex items-center" whileHover={{ x: 3 }}>
                <FiUser className="mr-1.5 text-orange-300" />
                {author}
              </motion.div>
            </div>

            <div className="flex items-center text-sm font-medium text-amber-400 hover:text-orange-300 transition-colors">
              Read More{" "}
              <motion.span
                className="ml-1.5"
                animate={{
    ...
    <section id="blog" className="pt-6 pb-8 md:pb-10 bg-[#121212]">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-[#d1c9c3] max-w-2xl mx-auto">
